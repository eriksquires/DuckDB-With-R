...
A feature to lean into is that dbt, DuckDB, and Postgres have extensions to read external databases (postgres, mysql, etc), files (parquet, csv, etc.) and remote sources via httpfs. These may eliminate the initial need to do an external ETL and keep your work 100% inside of DuckDB and by extension in dbt. Postgres, as an example, has at least a Foreign Data Wrapper which serves this purpose nicely.

Regardless of your data lake or desktop DB these options are all worth considering before reinventing the wheel with custom, brittle ETL scripts. Connections are a lot better than ETL jobs.
